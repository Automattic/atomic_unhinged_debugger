...
function add_filter( $hook_name, $callback, $priority = 10, $accepted_args = 1 ) {
	global $wp_filter;
	global $plugin_api_timer;
	AUD_Timer::instance()->start( __FUNCTION__, $hook_name );
	if ( ! isset( $wp_filter[ $hook_name ] ) ) {
		$wp_filter[ $hook_name ] = new WP_Hook();
	}

	$wp_filter[ $hook_name ]->add_filter( $hook_name, $callback, $priority, $accepted_args );
	AUD_Timer::instance()->stop( __FUNCTION__, $hook_name );
...
function apply_filters( $hook_name, $value, ...$args ) {
	...
	if ( isset( $wp_filter['all'] ) ) {
		...
		AUD_Timer::instance()->start( __FUNCTION__ . ':all', $hook_name );
...
function apply_filters_ref_array( $hook_name, $args ) {
	...
	if ( isset( $wp_filter['all'] ) ) {
		...
		AUD_Timer::instance()->start( __FUNCTION__ . ':all', $hook_name );
		_wp_call_all_hook( $all_args );
		AUD_Timer::instance()->stop( __FUNCTION__ . ':all', $hook_name);
	}
